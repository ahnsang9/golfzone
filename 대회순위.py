...
if __name__ == '__main__':
    ...
    def crawling(info):
        ...
        for i in range(len(rank)):
            info[0].append(rank[i].text.replace('T', ''))
            info[1].append(nickname[i].text)
    ...
    driver_path = 'C:\\Users\82105\PycharmProjects\대회순위\chromedriver.exe'
    ...
    wb = openpyxl.load_workbook('C:\\Users\82105\Desktop\\매장대회.xlsx')
    temp = wb.active
    ...
    for i in range(min(len(info2[1]), 5)):
        temp.cell(row=i+29,column=1).value = int(df['rank_min'][i])
    ...
    wb.save('C:\\Users\82105\Desktop\\매장대회(%d월%d일).xlsx' % (datetime.now().month, datetime.now().day))
